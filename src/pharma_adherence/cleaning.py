...
def clean_prescription_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['pharmacy_name'] = df['pharmacy_name'].str.replace(r'#\d+$','',regex=True)
    df['pharmacy_name'] = df['pharmacy_name'].replace({"cvs pharmacy":"cvs","walgreens pharmacy":"walgreens","csv":"cvs"})
    #TODO: copay_amount
    ...
    df.loc[(0 > df["proportion_days_covered"])|(1 < df["proportion_days_covered"]),"proportion_days_covered"]= np.nan
